...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Tracks</h1>
        <p class="text-gray-600 mt-1">Manage agenda tracks and categories</p>
    </div>
...
@if($tracks->isEmpty())
    <div class="bg-white rounded-lg shadow-sm p-12 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-800 mb-2">No Tracks Found</h3>
        <p class="text-gray-600 mb-4">Get started by creating your first track.</p>
        <a href="{{ route('admin.tracks.create') }}" 
...
@else
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                @if($track->description)
                                    <div class="text-xs text-gray-500">{{ Str::limit($track->description, 40) }}</div>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $track->agenda->title }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $track->sessions->count() }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">{{ $track->sort_order ?? 0 }}</div>
                    </td>
    ...
    <div class="mt-4 text-sm text-gray-600">
        Total: {{ $tracks->count() }} track{{ $tracks->count() !== 1 ? 's' : '' }}
    </div>
@endif
@endsection
