...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Category Types</h1>
        <p class="text-gray-600 mt-1">Manage category types for registration categories</p>
    </div>
    <div class="flex flex-col gap-2 sm:flex-row">
        @include('admin.components.parameter-bulk-import', ['parameter' => 'category-types'])
...
@if($categoryTypes->isEmpty())
    <div class="bg-white rounded-lg shadow-sm p-12 text-center">
        ...
        <h3 class="text-lg font-semibold text-gray-800 mb-2">No Category Types</h3>
        <p class="text-gray-600 mb-4">Get started by creating your first category type.</p>
        <a href="{{ route('admin.category-types.create') }}" 
...
@else
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                @if($type->description)
                                    <div class="text-sm text-gray-500">{{ Str::limit($type->description, 40) }}</div>
                                @endif
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <form action="{{ route('admin.category-types.toggle', $type) }}" method="POST" class="inline">
                            @csrf
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex justify-end space-x-2">
                            ...
                            <form action="{{ route('admin.category-types.destroy', $type) }}" 
                                  method="POST" 
    ...
    <div class="mt-4 text-sm text-gray-600">
        Total: {{ $categoryTypes->count() }} type{{ $categoryTypes->count() !== 1 ? 's' : '' }}
    </div>
@endif
@endsection
