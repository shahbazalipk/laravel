...
<div class="mb-6">
    <div class="flex items-center mb-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Product Type</h1>
            <p class="text-gray-600 mt-1">Update product type information</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-lg shadow-sm p-6">
    ...
    <form action="{{ route('admin.product-types.update', $productType->hash) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $productType->name) }}"
            ...
            <div>
                ...
                <div class="flex items-center space-x-3">
                    <input type="color" name="color" id="color" value="{{ old('color', $productType->color ?? '#6366f1') }}"
                           class="h-10 w-20 border border-gray-300 rounded cursor-pointer">
                    <input type="text" id="color-text" value="{{ old('color', $productType->color ?? '#6366f1') }}"
            ...
            <div>
                ...
                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $productType->sort_order) }}"
        ...
        <div class="mt-6">
            ...
            <textarea name="description" id="description" rows="3"
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $productType->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
