...
<div class="mb-6">
    <div class="flex items-center mb-4">
        ...
        <div>
            ...
            <p class="text-gray-600 mt-1">Update agenda details</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-lg shadow-sm p-6">
    <form action="{{ route('admin.agenda-management.update', $agenda) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       placeholder="e.g., Main Conference Agenda"
                       required>
                @error('title')
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          placeholder="Optional description of the agenda">{{ old('description', $agenda->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" 
                    ...
                    <option value="archived" {{ old('status', $agenda->status) === 'archived' ? 'selected' : '' }}>Archived</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-400">Draft agendas are not visible to attendees</p>
            </div>
        </div>
