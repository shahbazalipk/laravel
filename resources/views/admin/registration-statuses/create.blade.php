...
@section('title', 'Create Registration Status')

@section('content')
<div class="mb-6">
    <div class="flex items-center mb-4">
        <a href="{{ route('admin.registration-statuses.index') }}" 
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Create Registration Status</h1>
            <p class="text-gray-600 mt-1">Add a new registration status type</p>
        </div>
    </div>
</div>

<div class="bg-white rounded-lg shadow-sm p-6">
    <form action="{{ route('admin.registration-statuses.store') }}" method="POST">
        @csrf
        ...
        <div class="mt-6">
            ...
            <textarea name="description" 
                      ...
                      placeholder="Optional description for this status">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
