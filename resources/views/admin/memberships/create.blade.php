...
<div class="mb-6">
    <div class="flex items-center mb-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Create Membership</h1>
            <p class="text-gray-600 mt-1">Add a new membership type with verification method</p>
        </div>
    </div>
</div>
    ...
    <form action="{{ route('admin.memberships.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <select name="verification_type" 
                    ...
                    <option value="upload_file" {{ old('verification_type') === 'upload_file' ? 'selected' : '' }}>Upload File (CSV/TXT)</option>
                    <option value="third_party_api" {{ old('verification_type') === 'third_party_api' ? 'selected' : '' }}>Third Party API</option>
                </select>
        ...
        <div id="file-upload-section" class="mt-6 hidden">
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 bg-gray-50">
                <label for="membership_file" class="block text-sm font-medium text-gray-700 mb-2">
                    Membership File <span class="text-red-500">*</span>
                </label>
                <input type="file" 
                ...
                <p class="mt-2 text-xs text-gray-500">Upload a TXT or CSV file containing membership numbers (one per line)</p>
        ...
        <div class="mt-6">
            ...
            <textarea name="description" 
                      ...
                      placeholder="Optional description for this membership type">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
