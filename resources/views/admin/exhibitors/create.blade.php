...
<div class="mb-6">
    <div class="flex items-center mb-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Add Exhibitor</h1>
            <p class="text-gray-600 mt-1">Create a new exhibitor profile</p>
        </div>
    </div>
</div>
    ...
    <form action="{{ route('admin.exhibitors.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div id="content-basic" class="tab-content">
                ...
                <div>
                    ...
                    <select name="exhibitor_type_id" id="exhibitor_type_id"
                        ...
                        @foreach($exhibitorTypes as $type)
                            <option value="{{ $type->id }}" {{ old('exhibitor_type_id') == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">Format: JPG or PNG | Max size: 2MB</p>
            </div>
        </div>
        ...
        <div id="content-booth" class="tab-content hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="booth_type_id" id="booth_type_id"
                        ...
                        @foreach($boothTypes as $type)
                            <option value="{{ $type->id }}" {{ old('booth_type_id') == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="booth_number" id="booth_number" value="{{ old('booth_number') }}"
                ...
                <div>
                    ...
                    <input type="number" name="booth_size" id="booth_size" value="{{ old('booth_size') }}" step="0.01"
                    ...
                    @foreach($businessActivities as $activity)
                        <label class="flex items-center">
                            <input type="checkbox" name="business_activities[]" value="{{ $activity->id }}"
                                   {{ in_array($activity->id, old('business_activities', [])) ? 'checked' : '' }}
